...
| Hosting                                 | **Cloudflare Workers** (`@cloudflare/vite-plugin` + `wrangler`), Cron Triggers hitting `/api/cron/run/<jobId>` guarded by `CRON_SECRET` bearer — [2026-07-03-cloudflare-hosting.md](./decisions/2026-07-03-cloudflare-hosting.md) |                                                                                                                |
...
- Origin resolution: on Cloudflare Workers, set `BETTER_AUTH_URL` per environment (dashboard secrets) for each deployed hostname — production custom domain and each `*.workers.dev` preview; localhost uses the hardcoded trusted origin. Include the loopback-port reconciliation helper (rewrite a stale `localhost` URL's port to `process.env.PORT`) so parallel dev worktrees on different ports sign in without editing `.env`. Expose it as a client-safe subpath. See [2026-07-03-cloudflare-hosting.md](./decisions/2026-07-03-cloudflare-hosting.md).
