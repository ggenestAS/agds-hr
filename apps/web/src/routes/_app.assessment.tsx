...
import type { AssessCaseDetail, AssessReportRow } from "../server/people.shared.ts";
...
// The manager assessment (improve-ux plan): your reports — direct (either
// line) first, then indirect — with review-readiness status. Start the
// assessment once evidence collection is done (self-review submitted, no peer
// request pending); indirect reports carry a do-it-via-the-direct-manager
// warning. Self-review and peer content are toggleable next to the form.
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
                    ...
                    <>
                      ...
                      {row.caseId === undefined ? (
                        ...
                        <Button
                          ...
                          size="sm"
                          variant="secondary"
                          onClick={() => setSelectedId(row.caseId ?? null)}
                        >
                          View assessment
                        </Button>
                      ) : row.ready ? (
                        <Button
                          ...
                          onClick={() => setSelectedId(row.caseId ?? null)}
                        >
                          {activeId === row.caseId ? "Assessing…" : "Start assessment"}
                        </Button>
                      ) : (
                        <span
      ...
      {detail === null ? (
        ...
        <>
          {!detail.direct && (
            <div className="mt-5 rounded-[14px] border border-[var(--color-warning)]/40 bg-[var(--color-warning-surface)] px-4 py-3 text-[13px] text-[var(--color-warning)]">
              <strong>{detail.subjectName ?? detail.subjectEmail} is an indirect report</strong> —
              this assessment should normally be written by their direct manager. Proceed only if
              you have agreed to cover it.
            </div>
          )}
                        ...
                        {detail.peers.map((peer) => (
                          <div key={peer.requesteeEmail}>
                            <p className="mb-1 text-xs font-semibold text-muted-foreground">
                              From {peer.requesteeName ?? peer.requesteeEmail} ·{" "}
                ...
                {lowRating && (
                  <div className="flex items-start gap-3 rounded-xl border border-[rgba(233,75,60,0.28)] bg-[#fbf1ee] px-4 py-3">
                    ...
                    <label className="flex items-start gap-2 text-[12.5px] leading-relaxed text-[#8a3325]">
                      ...
                      />
                      A rating of 1 or 2 requires acknowledging that an improvement plan (P6) will
                      be triggered before the assessment can be submitted.
                    </label>
                  </div>
                )}
